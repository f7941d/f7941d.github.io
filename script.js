...
async function fetchData() {
    try {
        const response = await fetch(
            "https://raw.githubusercontent.com/f7941d/f7941d.github.io/refs/heads/main/jsons/messages.json"
        );
    ...
    try {
        const response = await fetch(
            "https://raw.githubusercontent.com/f7941d/f7941d.github.io/refs/heads/main/jsons/extra.json"
        );
...
function update() {
    ...
    for (let network in networks) {
        ...
        if (
            ...
            extra["unverified"][network] !== undefined &&
            networks[network]["passwords"].length == 1
        ) {
            ...
            if (
                ...
                if (networks[network]["passwords"][password] !== false) {
                    if (extra["verified"][network] !== undefined) {
                        ...
                        extra["unverified"][network] ==
                            networks[network]["passwords"][password] ||
                        (typeof extra["unverified"][network] == "object" &&
                            extra["unverified"][network].includes(
                                networks[network]["passwords"][password]
                            ))
                    ) {
                        networkPassword.className = "password incorrect";
